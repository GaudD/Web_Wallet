...
const DropDown: React.FC<DropDownProps> = ({ mnemonic, clearMnemonic }) => {
  const [isOpen, setIsOpen] = useState(false);
  const [currentMnemonic, setCurrentMnemonic] = useState("");

  // Set mnemonic to localStorage when component mounts
  useEffect(() => {
    if (!localStorage.getItem("mnemonic")) {
      localStorage.setItem("mnemonic", mnemonic);
    }
    setCurrentMnemonic(localStorage.getItem("mnemonic") || "");
  }, [mnemonic]);

  const seedPhrase = currentMnemonic.split(" ");
  ...
  return (
    <div className="relative w-10/12 pt-5 max-h-screen">
      ...
      <div className="flex justify-between pt-6">
        ...
        <div>
          <button onClick={clearMnemonic} type="button" className="py-2.5 px-4 me-2 mb-2 mt-[10px] text-sm text-white font-roboto bg-red-900 rounded-md hover:bg-red-950">
            Clear Wallet
          </button>
        </div>
      </div>
      ...
      <div>
        <Wallet mnemonic={currentMnemonic} />
      </div>
    </div>
  );
};
